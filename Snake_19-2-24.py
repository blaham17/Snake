    ...
    def HniSe(self):                                #HERNI CYKLUS
        ...
        if self.snake_x == self.food_x and self.snake_y == self.food_y:     #Kdyz bylo snezeno jidlo:
            ...
            if self.level == 20:
                ...
                self.bonus = 20
            self.Jidlo()
            self.rychlost -= (0.08 * self.rychlost)             #zvyseni rychlosti
            #self.timer2 = QTimer()
            #self.timer2.singleShot(1000)
            #self.timer2.stop()
            self.timer1 = QTimer()
            self.timer1.timeout.connect(self.HniSe)
    ...
    def mapa_5(self):
        ...
        for i in (6, 7, 8):
            for j in (6, 7, 8):
                self.pole[i][j] = -2
        okno.resize((self.n * self.p), (self.n * self.p + 20))
        #okno.setGeometry(200, 400, (self.n * self.p), (self.n * self.p + 20))
        self.snake_x = 1
        self.snake_y = 1
        self.next_state = "R"
        self.update()
